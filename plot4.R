...
## This is Project 1 of Course 4 - Exploratory Data Analysis, this produces plot 4 of 4 with a quad of plots
## by C. Bleile
...
# 5. Output the plot
png("plot4.png", width=480, height=480) #sets output device to plot2.png located in the working directory
par(mfrow=c(2,2)) #set the plot to a quad chart

# 5a. 1st plot (UL Quadrant)
plot(HHpowerSubset$POSIXdateTime, HHpowerSubset$Global_active_power, type="l", xlab="", ylab="Global Active Power")

# 5b. 2nd plot (UR Quadrant)
plot(HHpowerSubset$POSIXdateTime, HHpowerSubset$Voltage, type="l", xlab="datetime", ylab="Voltage")

# 5c. 3rd plot (LL Quadrant)
...
# 5d. 4th plot (LR Quadrant)
plot(HHpowerSubset$POSIXdateTime, HHpowerSubset$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")
